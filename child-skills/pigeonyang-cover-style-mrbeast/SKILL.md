...
13. Save the exact final generation prompt to a prompt file and run the mother skill's `scripts/verify_prompt_firewall.py` with `MrBeast` and common aliases passed as `--forbid`. If a portrait/reference image is supplied, pass `--require-identity-reference`. Do not generate if the script fails.
